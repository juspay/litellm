...
// Current time expressed as an IST wall-clock datetime-local string.
const nowISTLocal = (subtract?: { value: number; unit: moment.unitOfTime.DurationConstructor }): string => {
  const m = moment().utcOffset(IST_OFFSET);
  if (subtract) m.subtract(subtract.value, subtract.unit);
  return m.format("YYYY-MM-DDTHH:mm");
};

// Interpret an IST datetime-local string as IST wall time and return a UTC string for the API.
...
const utcToISTDisplay = (utcIso: string): string =>
