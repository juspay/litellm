...
@pytest.mark.asyncio
async def test_spend_logs_ui_can_skip_exact_count_for_fast_pagination(monkeypatch):
    from litellm.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from litellm.proxy.spend_tracking.spend_management_endpoints import (
        ui_view_spend_logs,
    )
    ...
    mock_prisma.db.query_raw = AsyncMock(return_value=rows)
    mock_prisma.db.litellm_spendlogs = MagicMock()
    mock_prisma.db.litellm_spendlogs.count = AsyncMock(return_value=51)
    ...
    sql_call = mock_prisma.db.query_raw.call_args[0]
    assert "COUNT(*) OVER ()" not in sql_call[0]
    assert sql_call[-2:] == (51, 0)
    mock_prisma.db.litellm_spendlogs.count.assert_not_called()
